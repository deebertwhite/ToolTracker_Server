...
- **Dashboard charts**: the Global Overview adds three lightweight, hand-rolled SVG charts (no charting library) — a Tool Status Breakdown donut, a Calibration Compliance donut (Compliant / Due within 30 days / Overdue), and a 30-day Checkout/Check-in Activity trend (`GET /api/dashboard/activity-trend`). See `renderDonutChart()`/`renderBarChart()` in `dashboard.js`.
