...
- **Calibration traceability history**: completing a calibration (the QA step above) requires a calibration provider/lab name and a certificate or reference number, not just a new due date — each cycle is a permanent row in `calibration_records` (`migrations/006_calibration_history.sql`), visible as a full history on the tool's admin detail card, not just its current due date. Aimed at the calibration-traceability language in 14 CFR § 21.137(f) ("directly traceable to NIST or other national/international standards organization") and the equivalent Part 145/AS9100 quality-system expectations — see [Security Notes](#security-notes) for the important caveat on what this does and doesn't establish.
