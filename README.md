...
- **Bulk barcode label export, with scope**: the Export Inventory card's Department/Toolbox picker downloads a ZIP of label images (`GET /api/tools/labels/export`, optional `?dept_id=`/`?box_id=`/`?qr_code=` — at most one, otherwise every non-retired tool) split into `datamatrix/`/`code128/` subfolders so a print run for a specific scanner/label stock doesn't require picking through a mixed pile. A single tool's pair is also one click away as "Download Both Labels (ZIP)" on its own detail card. The only prior way to get more than one off the Pi was pulling files by hand over SSH. Built on a hand-rolled ZIP writer (`scripts/lib/zip.js`, stored/uncompressed — the PNGs are already compressed) rather than a dependency, same reasoning as the hand-rolled PNG chunk in `scripts/lib/datamatrix.js`.
...
- **Automatic per-tool barcode labels (Data Matrix + Code 128)**: creating a tool (single-tool ingest form or CSV import) automatically generates *two* label PNGs for it -- a Data Matrix code and a linear Code 128 barcode, each with the human-readable ID and the tool's name on a second row below -- stored as `tools.barcode_image_url` / `tools.linear_barcode_image_url`, separate files/fields from `photo_url` (the tool's actual picture), and both viewable/selectable from its card in the admin panel. Both exist side by side because plenty of cheap USB/laser barcode scanners are 1D-only and physically cannot read a 2D symbol like Data Matrix at all -- this isn't a migration away from Data Matrix, just added compatibility. Renaming a tool (edit form or CSV re-import) regenerates both labels automatically so the name row never goes stale.
...
Every tool gets two label PNGs automatically the moment it's created -- a Data Matrix code (`generateBarcodeLabel()` in `server.js`, stored at `tools.barcode_image_url`) and a linear Code 128 barcode (`generateLinearBarcodeLabel()`, stored at `tools.linear_barcode_image_url`, using `generateLinearBarcodePng()` in `scripts/lib/datamatrix.js`) -- both viewable from the tool's card in the admin panel, with generous padding around the code and its human-readable ID since these are meant for on-screen viewing/selection, not precision engraving. Code 128 exists alongside Data Matrix, not instead of it, specifically for cheap 1D-only barcode scanners that can't read a 2D symbol at all. Both get the tool's name composited on as a second, smaller row below the ID (`addNameRow()`, shared by both formats) -- wrapped to two lines, ellipsized if it still doesn't fit -- and both regenerate automatically whenever the tool is renamed, so neither can drift out of sync. Run `npm run backfill-barcode-labels` any time to generate whichever label(s) a tool is still missing (safe to re-run; only touches tools missing at least one format), or `npm run backfill-barcode-labels -- --all` to rebase *both* labels for *every* non-retired tool from scratch (e.g. after a label-format change) -- overwrites existing files.
