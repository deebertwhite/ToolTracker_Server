...
- **Bulk barcode label export**: "Export All Barcode Labels (ZIP)" in the admin portal downloads every non-retired tool's label images in one archive (`GET /api/tools/labels/export`), split into `datamatrix/` and `code128/` subfolders so a print run for a specific scanner/label stock doesn't require picking through a mixed pile — the only prior way to get more than one off the Pi was pulling files by hand over SSH. Built on a hand-rolled ZIP writer (`scripts/lib/zip.js`, stored/uncompressed — the PNGs are already compressed) rather than a dependency, same reasoning as the hand-rolled PNG chunk in `scripts/lib/datamatrix.js`.
