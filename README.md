...
- **Mandatory shift audits**: each department must log a toolbox audit at least once per shift window (morning 04:00–14:00, afternoon 14:00–04:00 overnight) before anyone can check tools **out** of that department that shift. Check-ins are never blocked. A live audit-status widget shows the current window and each department's status on both the dashboard and the kiosk idle screen. See `getAuditWindowStart()` in `server.js`. An audit is a single physical-inspection attestation for the whole box, not a per-tool scan/checklist: the kiosk shows what's currently expected in it (status `In`; tools already checked out or already flagged are excluded and shown only as a count), the operator physically opens the box and confirms, checks an attestation box, and submits — logged per-tool in `audit_logs` same as before. Deliberately this simple until there's real per-box electronic scanning hardware to justify more; a specific tool problem goes through the separate Report Issue workflow instead. The kiosk's toolbox picker is grouped by department and limited to departments the logged-in person can access (home department, plus any granted — see the cross-department access grants feature below; unrestricted for `super_admin`), enforced server-side on submission (`getUserAccess()`) so it can't be bypassed by calling the API directly. Auditing a toolbox returns to toolbox selection rather than logging the operator out, so several boxes can be audited in one visit.
...
- **Calibration traceability history**: completing a calibration (the QA step above) requires a calibration provider/lab name and a certificate or reference number, not just a new due date — each cycle is a permanent row in `calibration_records` (`migrations/006_calibration_history.sql`), visible as a full history on the tool's admin detail card, not just its current due date. Aimed at the calibration-traceability language in 14 CFR § 21.137(f) ("directly traceable to NIST or other national/international standards organization") and the equivalent Part 145/AS9100 quality-system expectations — see [Security Notes](#security-notes) for the important caveat on what this does and doesn't establish. A tool's detail card also has a "+ Log Calibration" form (`POST /api/tools/:id/calibration-history`) for logging a calibration directly from the admin panel, independent of the kiosk QA transfer flow — this is how existing inventory (whose calibration dates were set by the ingest form, a direct edit, or CSV import, with no traceable record behind them at all) gets its history backfilled. Backfilling an older record never regresses a newer date already on file, tracked or not.
- **Calibration checkout lock closes a real gap**: checking out a calibrated tool now hard-stops on any of three conditions — calibration overdue (already existed), **no due date on file**, or **no `calibration_records` row on file** (a tool marked calibrated with a plausible-looking due date but zero actual calibration history behind it). Previously only the overdue case was checked, so a tool with a manually-typed due date and no real evidence of ever being calibrated could still be issued. The inventory tree also shows *why* a calibrated tool can't go out — "No due date" / "No certificate" / "Cal expired" in red — instead of a blank or generic date.
- **Calibration pass/fail + trace-back investigations**: both places a calibration gets logged (the kiosk QA transfer's "Mark Calibration Complete" form and the admin panel's "+ Log Calibration" form) now record a Pass/Fail result (`calibration_records.result`, `migrations/013_trace_investigations.sql`) instead of treating every logged calibration as an implicit pass. A **Fail** immediately back-dates the tool's due date to the failure itself, so the existing checkout hard-stop above blocks it right away with no separate check needed, and auto-opens a **trace-back investigation** (`trace_investigations`/`trace_reviews`) — one review row per task the tool was checked out for since its last *passing* calibration, since every one of those tasks is now suspect. A supervisor (`dept_admin`+) works through each review from the Reports workspace's **Trace-Back Investigations** card (record In Tolerance / Out of Tolerance / Not Applicable per task, then close with a conclusion — or force-close with an override reason if some reviews are waived), and can also open one manually from a tool's detail card for a suspected defect with no failed calibration behind it (e.g. a customer complaint).
- **Duplicate-serial-number guard**: adding or editing a tool rejects a serial number already registered to a different tool (case/whitespace-normalized, so `SN-1` and `sn-1 ` collide) — enforced by a partial unique index (`migrations/011_serial_number_uniqueness.sql`) so it holds for CSV import too, not just the ingest form. The error names the existing tool and, if it's away for calibration, suggests receiving it back instead of re-adding it.
- **Calibration calendar feed**: `GET /api/calendar/calibration.ics` publishes a token-gated, read-only iCalendar feed of every calibrated tool's due date — subscribe from Google/Outlook/Apple Calendar for auto-refreshing reminders, or download a snapshot. No login session on that endpoint (a calendar app polling a URL can't do interactive auth) — a single shared secret in `.env` (`CALENDAR_FEED_TOKEN`) is the entire access control, and an unset token always rejects rather than defaulting to open access. The subscribe URL is surfaced to `super_admin` only, in the Reports workspace.
- **Work orders (optional, dormant until used)**: an optional "Work Order" field on the kiosk checkout screen tags every tool checked out in that batch (`audit_logs.work_order`) — nothing requires it, and check-in inherits the value automatically from each tool's own last checkout, so an operator never retypes it. The Reports workspace's **Work Orders** card groups activity by that label: tool count, how many are still out, and a **Close Out** action once everything's back (blocked with a clear message while any tool remains out), with **Reopen** available afterward. Built now so the plumbing exists, without changing anything about how checkout/check-in already work for anyone who ignores the field — see `migrations/012_work_orders.sql`.
- **Calibration Cockpit** (Dashboard app): an isolated, read-only view of calibrated tools only, modeled on a colleague's separate calibration-tracking prototype's own status dashboard — a stat-tile summary (all/in/out/at QA/due soon/locked) drilling into a filterable "needs attention" table, each tool shown with a solid-color day-remaining or "LOCKED" pill (same >60/30-60/0-29/locked day bands as the reference prototype, extended to also lock on an open trace-back investigation). Purely a second lens over the same `GET /api/tools` data the existing Global Overview already uses — doesn't change how calibrated tools are tracked or checked out anywhere else. See `renderCockpitView()`/`cockpitDteBand()` in `dashboard.js`. Also fixed a latent date-timezone bug this surfaced: `GET /api/tools` now casts `cal_due_date`/`last_cal_date` to `::text` (same fix as the calendar feed) so every consumer gets an unambiguous `YYYY-MM-DD` instead of a server-timezone-dependent timestamp.
- **Toolbox single-scan checkout**: a toolbox already has its own barcode (unlike a drawer); scanning IT instead of an individual tool during checkout checks out every tool currently `In` inside it in one action, expanded server-side (`POST /api/transactions`) and flowed through the exact same per-tool calibration/investigation/transfer/audit checks as if each had been scanned individually — one bad tool in the box still rolls back the whole batch. Checkout only; a toolbox code at check-in is rejected (both client- and server-side) since "the whole box came back at once" doesn't make physical sense. Detected client-side by shape (`<dept prefix>BOX-<number>`, generated by `fetchNextBoxId()`), which is also why `POST /api/departments` now rejects a prefix code ending in "BOX" — it would make an ordinary tool barcode indistinguishable from a toolbox one.
- **Calibration certificate photo**: each `calibration_records` entry in a tool's Calibration History can now have a photo of the physical certificate attached (camera icon → existing photo-upload pipeline, `migrations/015_calibration_certificate_photo.sql`) — click the thumbnail to view full-size. Reuses the same generic `POST /api/upload` endpoint already used for tool/user/toolbox/drawer photos rather than a separate upload path.
- **Tool groups (kits/assemblies)**: a lightweight logical tag over existing tools — a named set (e.g. "Torque Wrench Kit #3") a shop tracks and uses together, assigned from a tool's own edit form (`tools.group_id`, one group per tool, `migrations/014_tool_groups.sql`). Purely a tracking layer — tools are still scanned in/out individually exactly as before. The Reports workspace's **Tool Groups** card shows each group's roll-up status at a glance: how many members are present/out/flagged, and how many are **blocked** from checkout right now (an open trace-back investigation on any member, or, for calibrated members, the same no-due-date/expired/no-certificate conditions that block an individual checkout — kept as one shared SQL condition so this can't silently drift out of sync with the real checkout gate). `dept_admin`+ can create/edit/delete groups; deleting one just ungroups its members rather than touching the tools themselves.
- **Dashboard charts**: the Global Overview adds four lightweight, hand-rolled SVG charts (no charting library) — a Tool Status Breakdown donut, a Calibration Compliance donut (Compliant / Due within 30 days / Overdue), a 30-day Checkout/Check-in Activity trend (`GET /api/dashboard/activity-trend`), and a Shift Audit Compliance trend over the last 14 shift windows (`GET /api/dashboard/audit-compliance-trend`), aggregated across departments with per-department detail available. See `renderDonutChart()`/`renderBarChart()`/`renderComplianceTrendChart()` in `dashboard.js`.
...
- **Tool incident lifecycle**: every Broken/Missing/Worn report is also a permanent row in `tool_incidents` (`migrations/007_tool_incidents.sql`) — last known status/location, who reported it, and (this was the real gap being closed) exactly who resolved it, when, and how. Previously an admin editing a tool's status back to `In` or `Retired` left no trace at all; now that auto-closes the open incident and logs it, and the tool's admin detail card has a dedicated "Resolve Incident" form (Found/Repaired vs. Write Off) for doing it with resolution notes attached.
...
- **Bulk barcode label export, with scope**: the Export Inventory card's Department/Toolbox picker downloads a ZIP of label images (`GET /api/tools/labels/export`, optional `?dept_id=`/`?box_id=`/`?qr_code=` — at most one, otherwise every non-retired tool) organized as `datamatrix|code128/small|medium|large/<qr_code>.png` so a print run for a specific scanner/label stock doesn't require picking through a mixed pile. A single tool's full set is also one click away as "Download All (ZIP)" on its own detail card, which also shows all six as a small size-by-format grid. The only prior way to get more than one off the Pi was pulling files by hand over SSH. Built on a hand-rolled ZIP writer (`scripts/lib/zip.js`, stored/uncompressed — the PNGs are already compressed) rather than a dependency, same reasoning as the hand-rolled PNG chunk in `scripts/lib/datamatrix.js`.
- **Scan feedback**: every barcode capture — camera or hardware/USB scanner, tool scan or badge login — plays a short synthesized beep (`playScanFeedback()` in `kiosk.js`/`admin.js`, a Web Audio oscillator tone, no audio file asset) plus a brief vibration where the platform supports it. Vibration is Android-only in practice: iOS Safari has never implemented the Vibration API, even installed as a PWA, so an iPhone/iPad (or a desktop) is silently audio-only rather than erroring.
- **Moving a tool between drawers/toolboxes/departments**: a tool's edit modal has a Department/Toolbox/Drawer cascade defaulted to its current location — reassigning it there (or editing the Drawer column and re-importing the CSV) relocates it.
- **Shadow board map**: a drawer's photo (already supported — see `drawers.photo_url`) can have each tool's spot marked on it (`tools.position_x`/`position_y`, fractional 0.0–1.0 so it survives the photo being re-uploaded at a different resolution — `migrations/009_tool_positions.sql`). Placed from the drawer's entity card in the admin panel (`renderPositionMap()` in `admin.js` — pick an unplaced tool, click its spot on the photo); viewable read-only from anywhere via the dashboard's "Shadow Board Map" page (`showDrawerMapView()` in `dashboard.js`), which colors every marker by that tool's live status (green = In, accent = Out, red = Missing/Broken/Worn) without needing to stand at the box. `PUT /api/tools/:id/position` is deliberately separate from the general tool edit endpoint — dragging a marker is a frequent, low-stakes action that shouldn't need the full edit form.
- **Account security**: PINs are bcrypt-hashed, brute-force attempts are throttled by individual badge (IP-based throttling is temporarily disabled — see [Security Notes](#security-notes)), and every admin action is re-checked against the database's current role on every request.
...
- **Automatic per-tool barcode labels (Data Matrix + Code 128, x Small/Medium/Large)**: creating a tool (single-tool ingest form or CSV import) automatically generates *six* label PNGs for it -- Data Matrix and a linear Code 128 barcode, each at three sizes -- every one with the human-readable ID and the tool's name on a second row below, all viewable from its card in the admin panel. Two formats exist side by side because plenty of cheap USB/laser barcode scanners are 1D-only and physically cannot read a 2D symbol like Data Matrix at all; three sizes exist because different tools need different physical label sizes and one compromise size doesn't fit everything (sizes anchored to `scripts/generate-size-test-sheet.js`'s own validated 3-20mm candidate range -- see `BARCODE_SIZES` in `server.js`). Renaming a tool (edit form or CSV re-import) regenerates all six automatically so the name row never goes stale.
...
Every tool gets six label PNGs automatically the moment it's created -- Data Matrix (`generatePngAtSize()`) and linear Code 128 (`generateLinearBarcodePng()`, both in `scripts/lib/datamatrix.js`), each at three sizes (`BARCODE_SIZES` in `server.js`: small/medium/large, anchored to `scripts/generate-size-test-sheet.js`'s own validated 3-20mm candidate range) -- all viewable from the tool's card in the admin panel, with generous padding around the code and its human-readable ID since these are meant for on-screen viewing/selection, not precision engraving. Two formats exist side by side specifically for cheap 1D-only barcode scanners that can't read a 2D symbol at all; three sizes exist because one compromise size doesn't fit every tool. `generateAllBarcodeLabels()` generates all six together and `saveBarcodeLabelUrls()` writes them in one UPDATE, so a tool's labels never end up with some sizes/formats present and others missing. Every one gets the tool's name composited on as a second, smaller row below the ID (`addNameRow()`, shared across all six) -- wrapped to two lines, ellipsized if it still doesn't fit -- and all six regenerate automatically whenever the tool is renamed. Run `npm run backfill-barcode-labels` any time to generate whichever a tool is still missing (safe to re-run; only touches tools missing at least one of the six), or `npm run backfill-barcode-labels -- --all` to rebase all six for *every* non-retired tool from scratch (e.g. after a label-format change) -- overwrites existing files. "medium" is this feature's original size and reuses its original two columns/filenames unchanged; small/large are new (`migrations/010_barcode_size_variants.sql`).
...
- **Brute-force lockout**: a per-badge, DB-backed lockout (`failed_pin_attempts`/`locked_until`) locks an individual account after repeated bad PINs regardless of source IP — this is the real defense and is always active. There's also an IP-based rate limiter (`authLimiter`) that throttles every PIN-checking endpoint as a coarser, secondary catch-all; it's **temporarily disabled** (2026-09-11) because its 20-attempts/15-min window was being hit during entirely legitimate use of a single shared kiosk (many people, one IP, across a shift). See the comment above `authLimiter`'s definition in `server.js` to re-enable it (ideally at a higher `max` sized for a shared walk-up device).
...
- **On FAA/regulatory compliance**: this app (calibration traceability, mandatory shift audits, dual-PIN sign-off, full audit trail) is built to *support* good tool-control and calibration-traceability practice, and specifically targets the calibration-traceability language in 14 CFR § 21.137(f). It is **not** a certification of compliance, and no software feature list can be one — actual compliance depends on the organization's own approved, documented procedures (e.g. a Repair Station Manual or Quality Manual) as accepted by the relevant FAA office, not on what a tool happens to log. Treat every "FAA-facing" feature here as infrastructure a qualified aviation quality/compliance reviewer can point to, not a substitute for that review.
- **Records retention**: `audit_logs`, `calibration_records`, and `tool_incidents` are retained **indefinitely** — nothing in this codebase ever deletes a row from any of them; `scripts/prune-old-logs.js` only rotates the plain-text hourly/daily log *files* under `logs/` (2-year retention, unrelated to the database), and the Google Drive backup script's 30-backup rotation only limits how many *restore points* are kept, not how much history is inside each one — every daily backup is a full dump containing the complete table history up to that point. In short: the live database is the permanent record by default, which already exceeds any reasonable minimum retention window until/unless a deliberate archival policy is defined later.
