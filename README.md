...
- **Mandatory shift audits**: each department must log a toolbox audit at least once per shift window (morning 04:00–14:00, afternoon 14:00–04:00 overnight) before anyone can check tools **out** of that department that shift. Check-ins are never blocked. A live audit-status widget shows the current window and each department's status on both the dashboard and the kiosk idle screen. See `getAuditWindowStart()` in `server.js`. A physical audit can be done either by scanning each tool's barcode or by hand via the per-tool Condition dropdown (Present/Missing/Broken) — scanning is a shortcut, not a requirement. The kiosk's toolbox picker is grouped by department and limited to departments the logged-in person can access (home department, plus any granted — see the cross-department access grants feature below; unrestricted for `super_admin`), enforced server-side on submission (`getUserAccess()`) so it can't be bypassed by calling the API directly. Auditing a toolbox returns to toolbox selection rather than logging the operator out, so several boxes can be audited in one visit.
