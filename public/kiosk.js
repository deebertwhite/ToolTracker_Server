...
/**
 * True if a scanned/queued code has a toolbox's shape ("<dept prefix>BOX-<number>",
 * generated by fetchNextBoxId() server-side -- an ordinary tool code never contains "BOX-",
 * see the prefix-code validation on POST /api/departments) rather than an individual tool's.
 * A cheap client-side string check, not a server round-trip, so scanning stays instant --
 * the real, authoritative check is the exact-match DB lookup POST /api/transactions does at
 * submit time (see its "3.5 TOOLBOX" step), this is only used for immediate UI feedback.
 */
function isToolboxCode(qr) {
    return qr.includes('BOX-');
}
 ...
 *
 * A toolbox's own barcode (unlike a drawer, which doesn't have one -- see
 * migrations/000_initial_schema.sql) can also be scanned here during checkout, as a
 * single-scan shortcut for "check out everything currently in this box": the actual
 * expansion into individual tools happens server-side (POST /api/transactions), this
 * function only recognizes the *shape* of a toolbox code client-side (see isToolboxCode())
 * so it can be queued distinctly and rejected up front during check-in, where scanning a
 * whole box doesn't make physical sense (the tools coming back could be from different
 * people at different times, not "the whole box at once").
 */
function handleToolScan() {
    ...
    const isBox = isToolboxCode(qr);
    if (isBox && pendingMode !== 'OUT') {
        showToast(`${icon('triangle-alert', 'icon-warning')} Toolbox barcodes can only be scanned for checkout, not check-in.`);
        input.value = '';
        return;
    }
...
async function submitTransaction(managerPin = null) {
    ...
    try {
        ...
        document.getElementById('override-modal').style.display = 'none';
        // data.tool_count (not batchQueue.length) since scanning a toolbox barcode can expand
        // one queued line into several actual tools -- see the 3.5 step in POST /api/transactions.
        showToast(`${icon('circle-check', 'icon-success')} Successfully processed ${data.tool_count} assets.`);
